...
def main():
    ...
    logging.info('Code started running at %s', datetime.now().isoformat())

    database = 'DRIVE'
    logging.info('get %d img from %s data', len(args.images), database)
    image_collections = [DataLoader(x, database) for x in args.images]
    mask_list = generate_line_mask_list(args.kernel, args.rotation)

    if args.train:
        train_model(image_collections, mask_list, args.kernel, False)
    else:
        print(int(args.images[0])) 
        classify_image(image_collections, mask_list, args.kernel, args.save,int(args.images[0]))
